...
namespace Backend.Repository
{
    public interface BookingRepository
    {
        ...
        Task<List<Booking>> GetByStudentId(int studentId, byte? status, DateOnly? date);
        Task<List<Booking>> GetByTeacherId(int instructorId, byte? status, DateOnly? date);
        ...
        Task<bool> HasBookingByAvailabilityId(int availabilityId);
        Task<List<Booking>> GetBookingsByMonth(int month, int year);
        Task Save();
    }
}